...
def evaluate_retrieval(pipeline, test_questions):
    ...
    for q in tqdm(test_questions, desc="Retrieval eval"):
        ...
        relevant_found = any(
            q.get("expected_topic", "").lower() in r.content.lower() for r in retrieved
        )
    ...
    # Calculate metrics
    recall = (
        sum(1 for r in results if r["relevant_found"]) / len(results) if results else 0
    )
...
def evaluate_generation(pipeline, test_questions):
    ...
    for q in tqdm(test_questions, desc="Generation eval"):
        try:
            response = pipeline.answer(
                q["question"], num_documents=5, include_explanation=True
            )
    ...
    if successful:
        ...
        high_confidence = sum(
            1 for r in successful if r["confidence_level"] == "high"
        ) / len(successful)
    else:
        avg_confidence = 0
        avg_sources = 0
...
def evaluate_xai(pipeline, test_questions):
    ...
    for q in tqdm(test_questions, desc="XAI eval"):
        try:
            ...
            supported = sum(
                1 for a in response.attributions if a.get("source") != "Unsupported"
            )
            coverage = (
                supported / len(response.attributions) if response.attributions else 0
            )
            ...
            results.append(
                {
                    ...
                    "explanation_provided": bool(
                        response.confidence.get("explanation")
                    ),
                }
            )
        except Exception as e:
    ...
    if successful:
        ...
        confidence_rate = sum(1 for r in successful if r["confidence_provided"]) / len(
            successful
        )
    else:
        avg_coverage = 0
...
def main():
    ...
    try:
        ...
        print(
            f"   Generation: Avg Confidence = {generation_results['avg_confidence']:.2%}"
        )

        xai_results = evaluate_xai(pipeline, test_questions)
        print(
            f"   XAI: Attribution Coverage = {xai_results['avg_attribution_coverage']:.2%}"
        )

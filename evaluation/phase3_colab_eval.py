...
def main():
    ...
    start = time.time()
    os.system(f"cd {PROJECT_ROOT} && python evaluation/run_ablation.py --n 97 --out-dir evaluation/results")
    ...
    start = time.time()
    os.system(f"cd {PROJECT_ROOT} && python evaluation/run_paper_eval.py --mode metrics --n 97")
    print(f"✓ Metrics complete ({time.time()-start:.1f}s)")
    
    # Task 3: BioMistral evaluation
    print("\n[3/3] Running BioMistral Evaluation...")
    start = time.time()
    os.system(f"cd {PROJECT_ROOT} && python evaluation/run_paper_eval.py --mode metrics --model biomistral --n 20")
    ...
    for fname in ["ablation.json", "metrics_full_tinyllama.json", "metrics_full_biomistral.json"]:

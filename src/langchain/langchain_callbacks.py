        ...
        **kwargs
        ...
        if self.chain_depth == 1:  # Top-level chain
            ...
            log_data = {
                "event": "chain_start",
                "run_id": self.run_id,
                "chain_name": serialized.get("name", serialized.get("id", ["unknown"])[-1]) if serialized else "unknown",
            }
    ...
    def _log(self, data: Dict):
        ...
        if self.log_to_console:
            ...
            if event == "chain_start":
                ...
                logger.info(f"📚 Retrieved {data.get('num_documents', 0)} documents (top score: {data.get('top_score', 0):.2f})")
...
class MetricsAggregator:
    ...
    def get_summary(self, last_n_hours: int = 24) -> Dict:
        ...
        return {
            ...
            "success_rate": chains_completed / (chains_completed + chains_failed) if (chains_completed + chains_failed) > 0 else 0,
            "avg_duration_seconds": total_duration / chains_completed if chains_completed > 0 else 0,
            "avg_confidence": sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0,
            "avg_docs_retrieved": sum(retrieval_counts) / len(retrieval_counts) if retrieval_counts else 0
        }

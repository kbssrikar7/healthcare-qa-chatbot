...
{answer}"""
    ...
    def format_example(self, question: str, answer: str) -> str:
        ...
        return self.INSTRUCTION_TEMPLATE.format(question=question, answer=answer)
    ...
    def prepare_dataset(self, qa_pairs: List[Dict]) -> Dataset:
        ...
        for qa in qa_pairs:
            text = self.format_example(question=qa.get("question", ""), answer=qa.get("answer", ""))
...
class QLoRATrainer:
    ...
    def setup_model(self):
        ...
        dtype = torch.float16 if use_cuda else getattr(torch, "bfloat16", torch.float32)
    ...
    def load_fine_tuned(self, path: str):
        ...
        base_model = AutoModelForCausalLM.from_pretrained(
            saved_config["base_model"], device_map="auto", trust_remote_code=True
        )
...
def main():
    ...
    # Prepare dataset
    dataset_builder = MedicalQADatasetBuilder(trainer.tokenizer, max_length=config.max_seq_length)

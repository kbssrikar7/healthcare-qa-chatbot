...
async def call_tool(name: str, arguments: dict) -> List[TextContent]:
    if name == "fda_drug_search":
        ...
        return await _fda_drug_recalls(
            arguments["drug_name"], arguments.get("limit", 5)
        )
    elif name == "fda_adverse_events":
        return await _fda_adverse_events(
            arguments["drug_name"], arguments.get("limit", 5)
        )
...
async def _fda_drug_search(drug_name: str) -> List[TextContent]:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        if resp.status_code != 200:
            return [TextContent(
                type="text", text=f"FDA API returned status {resp.status_code}"
            )]
        ...
        if not results:
            return [TextContent(
                type="text",
                text=f"No FDA label found for '{drug_name}'"
            )]
...
async def _fda_drug_recalls(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        if resp.status_code != 200:
            return [TextContent(
                type="text", text="No recall data found or API error."
            )]
        ...
        if not results:
            return [TextContent(
                type="text",
                text=f"No recalls found for '{drug_name}'"
            )]
...
async def _fda_adverse_events(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        if resp.status_code != 200:
            return [TextContent(
                type="text", text="No adverse event data found."
            )]
        ...
        if not results:
            return [TextContent(
                type="text",
                text=f"No adverse events found for '{drug_name}'"
            )]
        
        output = [f"Top Adverse Events for {drug_name.upper()} (FDA FAERS):"]
        for r in results[:limit]:
            output.append(
                f"  - {r.get('term', 'N/A')}: {r.get('count', 0)} reports"
            )
        return [TextContent(type="text", text="\n".join(output))]

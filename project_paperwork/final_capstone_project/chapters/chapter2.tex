...
Researchers like Guu et al.~\cite{guu2020realm} baked retrieval directly into pre-training, but at a massive computational cost. Like most \ac{rag} setups, I used the modular pipeline from Lewis et al. because it treats retrieval and generation as separate, swappable components.

I used \ac{rag} instead of fine-tuning because it lets me update the knowledge base without retraining the entire model. When new data arrives, you just index the documents and the existing model continues to work. Fine-tuning alone would require an expensive retrain every time the source data changes.
...
I split the work into six phases, starting with environment setup and a baseline performance check in Phase 0. In Phase 1, I focused on local speed improvements like \ac{lru} caching for embeddings, BM25 pickle caching, and per-stage latency tracking.

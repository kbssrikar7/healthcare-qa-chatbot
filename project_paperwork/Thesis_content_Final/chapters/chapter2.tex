...
Guu et al.~\cite{guu2020realm} pushed this further by embedding retrieval into the pre-training phase. While REALM couples retrieval and generation more closely, it requires substantial training resources. I chose the simpler, modular architecture proposed by Lewis et al. because it allows independent swapping of the retrieval and generation components.

The \ac{rag} architecture offers a major practical advantage over fine-tuning alone: updating the knowledge base does not require retraining the language model. When new data arrives, you just index the documents and the existing model continues to work. Fine-tuning alone would need an expensive retrain every time the source data changes.
...
I structured the execution into six phases. Phase 0 established the environment and baseline metrics. Phase 1 tackled local performance bottlenecks. I added \ac{lru} caches for embeddings and dumped the BM25 index to disk to speed up cold starts. I also built latency tracking into the pipeline and added a warmup script for the API.

...
\begin{enumerate}
    ...
    \item \textbf{Grounding gate}: The \texttt{\_check\_answerability()} method applies a three-part test before generation is permitted. First, an adaptive threshold compares each candidate retrieved document's score against \texttt{max(abs\_floor,\allowbreak{} adaptive\_threshold\_ratio $\times$ top\_score)}, meaning the qualifying bar scales dynamically with the best match's quality rather than sticking to a rigid cutoff. Second, a minimum-count check rejects results where too few documents cross that threshold. Third, a term-overlap check requires that at least 30\% of the query's medical terms appear somewhere in the retrieved passages. If any test fails, the pipeline abstains from generating text entirely. Instead, it falls back to \texttt{\_build\_extractive\_answer()}, selecting the single most relevant passage from the set and returning it verbatim as a conservative, source-backed response. This hard "abstain-and-extract" boundary acts as the primary safety mechanism against hallucinating answers when the knowledge base lacks adequate support.

...
from tests.conftest import MockRetriever, MockLLM, MockDocument
...
class TestLangChainPipeline:
    ...
    def test_langchain_grounding_gate_with_low_rrf_scores(self, mock_llm):
        ...
        is_ans = result.get("is_answerable", True) if isinstance(result, dict) else getattr(result, "is_answerable", True)
    ...
    def test_langchain_unanswerable(self, mock_llm):
        ...
        is_ans = result.get("is_answerable") if isinstance(result, dict) else getattr(result, "is_answerable", None)
...
class TestHallucinationDetector:
    ...
    def test_detect_grounded_answer(self):
        from src.xai.hallucination_detector import HallucinationDetector
        detector = HallucinationDetector()
        docs = [{"content": "Diabetes is a chronic metabolic disease affecting blood sugar levels."}]
        answer = "Diabetes is a chronic metabolic disease. It affects blood sugar levels in the body."
